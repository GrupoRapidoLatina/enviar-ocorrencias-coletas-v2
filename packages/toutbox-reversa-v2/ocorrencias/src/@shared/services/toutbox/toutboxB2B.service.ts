...
export class ToutboxB2BService implements IToutboxService {
  private apiKey: string = ENV.TOUTBOX_B2B_API_KEY;
  constructor(
  ...
  async sendOccurrence({
    ...
    const response = await this.httpService.post<
      Omit<SendOccurrenceCommand, "dataForLog" | "setFile">,
  ...
  async scheduleOrder(command: ScheduleOrderCommand, orderId: string) {
    const response = await this.httpService.post<
      ...
      IScheduleResponse
    >(`/Parcel/Pickup?mktp_order_id=${orderId}`, command, {
      headers: {
        Authorization: `${this.apiKey}`,
      },
    });
  ...
  async sendEquipments(command: SendEquipmentsCommand) {
    ...
    >(
      `/Parcel/Product?courier_id=81&mktp_order_id=${command.products[0].trackingNumber}`,
      command,
      {
        headers: {
          Authorization: `${this.apiKey}`,
  ...
  async sendCollectedEquipments(
    ...
    const response = await this.httpService.put<
      ...
      ISendEquipmentResponse
    >(
      `/Parcel/Product/Collected?courier_id=81&mktp_order_id=${orderId}`,
      {
        ...command,
      },
      {
