...
export class DrizzleOrderHistoryRepository implements IOrderHistoryRepository {
  ...
  async listLatestOrdersHistories(): Promise<LatestOrderHistory[]> {
    try {
      ...
      const isDay = actualHour > 6 && actualHour <= 18;
      const isNight = actualHour > 18 && actualHour <= 22;

      const limit = isDay ? 50 : isNight ? 75 : 150;
  ...
  async findByCodigoTelefonica(
    ...
    const result = await this.drizzle
      ...
      .where(
        and(
          eq(schemas.orderHistorySchema.numero_ba, numbero_ba),
          eq(
            schemas.orderHistorySchema.codigo_telefonica,
            `${codigo_telefonica}`
          )
    ...
    return new OrderHistory(result[0]);
  }

  async increaseRetryCount(id: number) {
    try {
      const result = await this.drizzle
        .update(schemas.orderHistorySchema)
        .set({
          resinc: sql<number>`COALESCE(${schemas.orderHistorySchema.resinc}, 0) + 1`,
        })
        .where(eq(schemas.orderHistorySchema._id, id));
